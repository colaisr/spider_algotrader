...
def update_market_data(_tickers, _update_times, _research_error_tickers, _error_tickers):
    ...
    for t in _tickers:
        try:
            ...
            _url = server_url + "test/updatemarketdataforcandidatespider_test"
...
def spyder():
    ...
    test_tickers = tickers[:num]
    # already_updated_tickers = 0
    error_tickers = []
    research_error_tickers = []
    update_times = []

    already_updated_tickers, error_status = update_market_data(test_tickers, update_times, research_error_tickers, error_tickers)

    num_of_tickers = len(test_tickers)

    if len(error_tickers) > 0 or len(research_error_tickers) > 0:
        test_tickers = []
        test_tickers = error_tickers
        error_tickers = []
        if len(research_error_tickers) > 0:
            for item in research_error_tickers:
                for k, v in item.items():
                    test_tickers.append(k)
            research_error_tickers = []
        already_updated_tickers, error_status = update_market_data(test_tickers, update_times, research_error_tickers, error_tickers)
...
def updatemarketdataforcandidate(t):
    _data = urllib.parse.urlencode({"ticker_to_update": t})
    _data = _data.encode('ascii')
...
updatemarketdataforcandidate('FGNA')
